...
namespace WishlistApi.Infrastructure
{
    ...
    public class RadixEncoding
    {
        ...
        private readonly BigInteger _radixBig;
        ...
        public int Radix => _digits.Length;
        ...
        public string Encode(byte[] bytes)
        {
            ...
            var resultLength = EncodingCharsCount(bytes.Length);
            ...
            while (!dividend.IsZero)
            {
                ...
                dividend = BigInteger.DivRem(dividend, _radixBig, out remainder);
                var digitIndex = Math.Abs((int) remainder);
                result.Add(_digits[digitIndex]);
            }

            if (IncludeProceedingZeros)
                for (var x = result.Count; x < result.Capacity; x++)
                    result.Add(_digits[0]); // pad with the character that represents 'zero'
        ...
        private void DecodeImplPadResult(ref byte[] result, int padCount)
        {
            if (padCount > 0)
            {
                var newLength = result.Length + DecodingBytesCount(padCount);
        ...
        {
            var padCount = 0;
            for (var x = 0; x < chars.Length; x++, padCount++)
                if (chars[x] != _digits[0]) break;

            var result = DecodeImpl(chars, padCount);
